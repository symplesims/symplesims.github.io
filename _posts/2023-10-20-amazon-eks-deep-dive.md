...
EKS를 경험하며 다른 사람에게 EKS를 어떻게 설명하면 좋을지 고민한적이 있는데 지난 AWS re:Invent 2021 에서 [Deep dive on Amazon EKS](https://youtu.be/cipDJwDWWbY) 영상이 가장 좋은것 같습니다.

EKS 관련하여 제 자신이 가장 처음 궁금해하던 질문이면서 다른 사람들이 제게 빈번하게 했던 질문으로 Kubernetes 클러스터를 Native로 구성하기 보다 왜 EKS를 사용하는지? 였습니다. 
...
사용자가 EKS를 사용해야하는 가장 큰 이유 3가지를 꼽으라면 저는 보안, 안정성, AWS 서비스와의 통합을 이야기 하겠습니다.

실제로 Kubernetes 클러스터를 운영하는것은 아주 큰 모놀리스 서비스플랫폼을 운영하는것처럼 간단한 작업이라도 고려해야 할 영향도가많고, 이로 인한 오버헤드가 상당히 많습니다.  
...
EKS 제품 원칙으로 보안, 태생 부터 상용서비스, 원할한 AWS 서비스와 통합, 바닐라 업스트림, 커뮤니티와 동반 성장 입니다.  

이 원칙들로 EKS 클러스터가 적은 오버헤드로 더 나은 효율을 위해 AWS 가 가지는 보안, 운영, 확장에 관련된 전문 기술 들을 활용하고 있습니다. 
...
AWS 에서 EKS 제품에 가장 많은 노력을 쏟는 영역이 보안 입니다. 보안 관련된 업스트림에서 OS 및 API 패치와 보안 관련된 기술들과의 통합 그리고 변경사항에 대한 테스트로 많은 시간을 할애하고 있습니다.  
...
위 다이어그램은 워크로드를 위한 Data-Plane 을 Self Managed 방식과 Fargate 방식을 사용했을때 AWS 책임 공유 모델을 설명 합니다.

공통적으로 Control-Plane을 AWS가 책임지고 있습니다. 만약 여러분이 Native Kubernetes 클러스터를 직접 구현한다면 가장 큰 운영 오버헤드가 이 부분에서 발생합니다.

Control-Plane은 Kubernetes 운영을 위한 핵심 컴포넌트인 API 서버, 스케줄러, 컨트롤러 관리자, etcd로 구성됩니다. EKS는 이것과 관련된 보안을 우선적으로 담당합니다.  
여기에는 개선된 업스트림에 보안관련된 문제가 있는지 검증하고 AWS KMS를 통한 암호화를 통해 etcd 등 중요한 정보를 암호화 하여 관리하고 있습니다.
...
Data-Plane은 서비스 워크로드를 위한 컴포넌트로 사용자를 위한 민감한 데이터와 전송 트래픽 관련된 보안이 적용됩니다. 다음은 Data-Plane 을 위한 보안 항목입니다.  
- Pod를 위해 제한된 IAM 엑세스를 정책을 적용합니다. (인터넷 서비스를 위해 Worker Node 에 IAM 액세스 정책을 할당하지 않습니다.)
- 외부에 저장되는 민감한데이터를 위해 Secrets Manager를 사용하고 암호화하고 Secrets Manager CSI 드라이버로 액세스 할 수 있습니다.
- 암호화 통신을 위해 AWS Certificate Manager 로 인증서를 만들고 TLS 프로토콜을 적용 합니다. Public 구간은 기본이며 Private 구간은 private TLS Issuer 플러그인을 제공하여 private 구간도 TLS 를 적용할 수 있습니다.  
...
- Private 앤드포인트: VPC 내부에서만 액세스 할 수 있으므로 강화된 보안 방식입니다. 상용 서비스를 제공한다면 Private 앤드포인트로 EKS 를 구성하는것을 권장 합니다.
...
- 워커노드를 위한 보안으로 컨테이너를위한 OS 최적화된 관리형 AMI 사용하면 좋습니다. 특히 Bottlerocket은 Linux 기반으로 컨테이너 런타임이 최적화되어있는 가장 최신의 기술입니다.  
- 워커노드는 상태를 변경할 수 없도록 운영하는것이 좋습니다. 클러스터 버전업이나 OS 보안 패치 등 새로운 AMI로 변경하는 경우에 현재 운영중인 워커노드를 수정하는 전략이 아닌 새로운 EC2 워커 노드를 생성하고 워크로드를 이전하는 전략이 좋습니다.
- 관리를 위해 SSH 포트를 오픈하는것 보다 AWS SSM을 사용합니다.
- Custom AMI를 사용하는경우 AWS CIS 벤치마크를 통해 검증하는게 좋습니다.
- 가능하다면 워크로드를 책임지는 Data-Plane 역시 EC2 워커노드가 아닌 AWS Fargate로 구성하면 AWS에 책임을 전가할 수 있습니다.
...
- IAM 정책 적용: 워커 노드에 IAM 정책을 구성하는 것보다 각각의 Pod 애플리케이션에 대한 IAM 정책을 별도로 구성하는것이 좋습니다.  
  여기에는 최소권한 원칙을 준수하는 IAM 정책 생성과 쿠버네티스 SeviceAccount 를 생성하여 IAM 과 바인딩 합니다.

- NetworkPolicy 를 통한 트래픽 제한: Pod 내부로 들어오거나(Ingress) 외부로 나가는(Egress) 트래픽을 허용하고 거부하는 정책을 설정합니다.  
  NetworkPolicy는 CIDR 및 포트를 통해 Block 및 제외할 수 있지만 몇가지 제한이 있습니다. 예를 들어 노드 단위 설정, TLS 설정, 다중포트 적용 등에서 불가능합니다.
...
위의 다이어그램과 같이 하나의 EKS 클러스터에 여러팀(A,B,C)이 운영하는 워크로드의 경우 각 팀을 위한 RBAC 및 네임스페이스를 사용하여 격리(논리적)하고 쿠버네티스 할당량 및 범위를 사용하여 컴퓨팅 리소스를 제어하는것이 좋습니다.

- 네임스페이스간 트래픽을 모두 거부하는 네트워크 기본 정책을 적용할 수 있습니다.
- RBAC을 통해 각 팀을 위한 제한된 액세스 정책을 적용하고 컨트롤 할수있습니다.
- v1.20 이상인경우 API 서버의 요청수를 제한할 수 있습니다.
- OPA(Open Policy Agent) 등 Gatekeeper 와 같은 도구를 사용하여 팀이 실행할 수 있는 항목을 거버넌스 정책으로 정의하고 제한할 수 있습니다.
- VM 레벨로 격리화가 이루어지는 Fargate 를 사용하는 것도 좋습니다.  

참고로, Tenant 고객에게 ML과 같은 무거운 워크로드를 제공하는경우, Kubernetes 자체는 싱글테넌트 오케스트레이터이므로 Namespace 와 RBAC 이 완전하게 격리되도록 별도의 클러스터를 생성하여 제공하는것이 좋습니다.
...
EKS 는 이미 산업 규제 대상의 여러컴플라이언스 표준을 준수하고 인증 받았습니다. 
...
EKS 가 고가용성 서비스를 보장하고 애플리케이션의 안정성을 보장하는 방법을 소개 합니다. 
...
Pod 에 관한 스케일링 정책을 설정했다면 워커 노드를 위한 자동 확장을 정책을 설정할 수 있습니다. AWS 관리형 노드 그룹은 EC2 Auto Scaling 그룹과 통합되어 워커 노드를 자동으로 확장 합니다.  
또 다른 워커노드 확장 정책으로 Karpenter 플러그인을 사용할 수 있습니다. 이는 Kubernetes Native 한 방식으로 동작하며 대규모 워크로드에서 워커노드를 아주 빠르게 확장 및 축소를 할 수 있습니다.   
마지막으로 워커 노드를 전혀 신경 쓰고 싶지않으면 Fargate를 사용하면 됩니다.  
...
EKS는 VPC-CNI(Container Network Interface) 플러그인을 사용할 수 있습니다. VPC-CNI의 가장큰 장점은 네트워크 성능과 함께 EKS 워커 노드와 Pod 의 네트워크 CIDR 대역이 VPC CIDR 대역과 같다는 것입니다.
...
- EBS와 같은 퍼시스트볼륨을 사용하고 기존 데이터를 유지해야하는 워커노드를 업그레이드 하는 경우 특별히 교체에 주의해야 합니다. PV(PersistentVolume) 리소스를 만들 때 persistentVolumeReclaimPolicy 옵션을 "Retain" 으로 하여야 합니다.  
...
[Deep dive on Amazon EKS](https://youtu.be/cipDJwDWWbY)는 EKS에 대해 통찰력 있고 꼭 필요한 내용을 다루고 있어서 Kuberneties의 기본을 이해하는 사람에게 더할라위없이 좋은 영상인 것 같습니다.  
